...
def is_task_completed(task_dir: str) -> bool:
    ...
    """
    try:
        # 1. 如果路径本身不存在，或者因为 Windows 路径限制导致无法访问，直接返回 False
        ...
        # 捕获 [WinError 3] 或其他路径相关的系统错误
        print(f"ℹ️ 路径暂不可达或不存在: {task_dir}，标记为未完成。")
        return False
            ...
            with open(os.path.join(task_dir, file_name), 'r', encoding='utf-8') as f:
                ...
                for act in actions:
                    if isinstance(act, dict):
                        if "done" in act: return True
                    elif isinstance(act, str):
                        if "done" in act.lower(): return True
...
async def run_single_task(goal: str, args, vision_only: bool = True, base_agent_dir: str = "agent_data"):
    ...
    # 如果你确定文件夹名可能还有空格，这里再保底 strip 一次
...
async def main():
    ...
    #     model_name="claude_edit",
    #     gen_model="_claude_edit",
    #     target_url="http://localhost:7000",
    #     task_n=1,  
    #     md_path=None 
    # )
    base_agent_dir = "D:\\dyad\\run\\agent_data"
    os.makedirs( f"D:\\dyad\\run\\agent_data\\{args.model_name}\\TEXT", exist_ok=True)
    # 1. 检查文件路径
    md_path = args.md_path or rf"D:\\dyad\\run\\web\\task_md\\{args.subdomain}_tasks.md"
    json_path= rf"D:\\dyad\\run\\agent_data\\{args.model_name}\\defect_tasks.json"

    print(f"🔍 正在检查任务文件: {md_path}")
    

    if not os.path.exists(md_path) or not os.path.exists(json_path):
        print(f"❌ 找不到任务文件: {md_path} or {json_path}")
        return
    ...
    with open(json_path, 'r', encoding='utf-8') as f:
        try:
            ...
            # --- 关键修改：对 Key 进行同步截断处理 ---
            processed_defect_keys = {get_safe_dir_name(k) for k in defect_data.keys()}
            
            print(f"📂 已加载并预处理 JSON 配置文件，包含 {len(processed_defect_keys)} 个目标 Key")
        except Exception as e:
    ...
    for goal in task_goals:
        ...
        # 此时 safe_goal 经过 get_safe_dir_name 处理，长度和格式与 processed_defect_keys 完全一致
        safe_goal = get_safe_dir_name(goal)
        
        if safe_goal not in processed_defect_keys:
            print(f"🚫 过滤 (不在 JSON 列表中): {safe_goal[:30]}...")
            continue
            
        # 生成路径（这里不要再用 Path.resolve() 了，直接拼接绝对路径）
