...
# --- 配置部分 ---
GEN_MODEL = "lovable_edit"
# 新增的 JSON 路径参数

json_file_path = f"D:\\dyad\\run\\web\\app_status\\app_status_{GEN_MODEL}.json"
...
def load_configs(file_path):
    ...
    for item in data.get("apps", []):
        ...
        configs.append({
                    ...
                    "md_path": f"D:\\dyad\\run\\web\\task_md\\{item['app']}_tasks.md"
                })
...
async def run_pipeline(config):
    ...
    scripts = [
        r"D:\dyad\run\pipeline\run\3_run_edit.py",
    ]
